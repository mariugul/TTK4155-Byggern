...
void oled_init() // Initialize OLED
{
    ...
    oled_print("WELCOME!");
    oled_pos(4, 0); // Print in the middle of the screen
    oled_print_inv("*ENTER GAME*");

    // Wait to display welcome screen
    //oled_print_logo();    // To be added
    _delay_ms(3000);
...
// Clears screen and put cursor at home position (origin)
...
{
    uint8_t print_char = c - 32;    // Start at space character
